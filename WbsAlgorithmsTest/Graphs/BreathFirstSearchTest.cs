...
namespace WbsAlgorithmsTest.Graphs
{
    ...
    public class BreathFirstSearchTest
    {
        ...
        public void SingleComponentTest()
        {
            ...
            Assert.That(BreathFirstSearch.ExploreIteratively(g, 0), Is.EqualTo(expected).AsCollection);

            // The vertex 5 is the sink. We are not able to reach any other vertices.
            expected = new List<int> { 5 };
            Assert.That(BreathFirstSearch.ExploreIteratively(g, 5), Is.EqualTo(expected).AsCollection);

            // From the vertex 3 we can reach vertices 4 and 5.
            expected = new List<int> { 3, 4, 5 };
            Assert.That(BreathFirstSearch.ExploreIteratively(g, 3), Is.EqualTo(expected).AsCollection);
        }
        ...
        public void MultipleComponentsTest()
        {
            ...
            Assert.That(BreathFirstSearch.ExploreIteratively(g, 1), Is.EqualTo(expected).AsCollection);

            // From vertex 2 we can reach vertices 5, 8, 6, 0, 3 (two components).
            expected = new List<int> { 0, 2, 3, 5, 6, 8 };
            Assert.That(BreathFirstSearch.ExploreIteratively(g, 2), Is.EqualTo(expected).AsCollection);
        }
    }
}
